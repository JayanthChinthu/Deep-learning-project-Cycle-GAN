...
def load_checkpoint(opts):
    ...
    """
    G_XtoY_path = os.path.join(os.getcwd(),opts.load, 'G_XtoY.pkl')
...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        m = len(labels_X)
        n = len(labels_Y)
        D_X_loss = 1/m * torch.sum((D_X(images_X)-1)**2)
        D_Y_loss = 1/n * torch.sum((D_Y(images_Y)-1)**2)
        ...
        D_X_loss = 1/n * torch.sum(D_X(fake_X.detach())**2)

        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_XtoY(images_X)

        # 5. Compute the loss for D_Y
        D_Y_loss = 1/m * torch.sum(D_Y(fake_Y.detach())**2)
        ...
        g_loss = 1/n * torch.sum((D_X(fake_X)-1)**2)

        if opts.use_cycle_consistency_loss:
            reconstructed_Y = G_XtoY(fake_X)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = 1/m * torch.sum((images_Y - G_XtoY(fake_X))**2)
        ...
        g_loss = 1/m * torch.sum((D_Y(fake_Y)-1)**2)

        if opts.use_cycle_consistency_loss:
            reconstructed_X = G_YtoX(fake_Y)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = 1/n * torch.sum((images_X - G_YtoX(fake_Y))**2)
            g_loss += cycle_consistency_loss
...
def create_parser():
    ...
    parser.add_argument('--sample_dir', type=str, default='samples_cyclegan/pretrained/withCon')
    parser.add_argument('--load', type=str, default='pretrained')
